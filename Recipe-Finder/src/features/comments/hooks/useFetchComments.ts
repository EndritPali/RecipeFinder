...
import { UseFetchCommentsOptions } from '@/types/comment';
...
const fetchComments = async (page = 1, pageSize = 15) => {
  ...
  const mappedComments = data.data.map((comment: any) => ({
    id: comment.id,
...
export const useFetchComments = (page: any, pageSize: any, options: UseFetchCommentsOptions = {}) => {
