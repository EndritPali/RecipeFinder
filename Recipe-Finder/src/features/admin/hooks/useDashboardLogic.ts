...
export function useDashboardLogic(
  ...
  const handleRecipeTableChange = useCallback((paginationConfig: any) => {
    setRecipePage(paginationConfig.current);
    setRecipePageSize(paginationConfig.pageSize);
  }, []);

  const handleUserTableChange = useCallback((paginationConfig: any) => {
  ...
  const dataSource = useMemo(() => {
    ...
    if (isUserDashboard) {
      ...
      return users.filter((item: any) => item.key !== user?.id);
    }
  ...
  const userPagination = {
    ...
    showTotal: (total: number) => `${total} User(s) in total`,
  };
  ...
  const recipePagination = {
    ...
    showSizeChanger: true,
    showTotal: (total: number) => `${total} Recipe(s) in total`,
  };
